...
3. **Index advice reuses the Phase-33 pipeline, not a new one (principles §2, §4, §8).** From a slow
   statement the user can request **index suggestions**: Prost first attempts a plain `EXPLAIN` (Phase 26)
   and feeds it into `AiService.suggestSchemaChanges` (Phase 33). Because both workload catalogs normalize
   literals into placeholders, an un-replayable statement falls back to normalized SQL + live schema/index
   metadata with a visible disclosure. Typed `createIndex` candidates are re-validated by
...
4. **Reads are bound and safe (principles §2, §4).** Snapshot queries are parameterized/bounded; the panel
   ...
   read-only `EXPLAIN` (never `ANALYZE` from this panel).

## Backend (`apps/api`)

### Driver + a perf read (`PerformanceModule`)
...
- A **Performance** ops panel (hidden when `supportsPerfInsights` is false; configured-engine runtime
  gaps render an actionable unavailable state): a sortable
...
The bundled demo PostgreSQL now preloads and creates `pg_stat_statements`; the demo MySQL user receives
read-only `performance_schema` access. Existing Docker volumes need to be recreated or configured manually
because initialization scripts only run for a new volume.

`pnpm -w typecheck`, `pnpm -w build`, `pnpm -w lint`, `pnpm -w test` all pass.
